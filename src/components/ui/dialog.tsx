...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Popup
      ...
      className={cn(
        // Common base styles
        "fixed z-50 grid w-full gap-0 border border-border/60 bg-popover/95 p-0 shadow-2xl transition-all duration-300 overflow-hidden",
        // Mobile: slide up from bottom, full width, rounded top corners
        "bottom-0 left-0 right-0 rounded-t-[1.5rem] border-b-0",
        "data-[state=open]:animate-in data-[state=closed]:animate-out",
        "data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom",
        "data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
        // Desktop: centered, max width, rounded all around
        "sm:bottom-auto sm:left-[50%] sm:top-[50%] sm:max-w-lg sm:translate-x-[-50%] sm:translate-y-[-50%] sm:rounded-[1rem] sm:border-b",
    ...
    >
      ...
      <div className="mx-auto mt-3 h-1.5 w-12 rounded-full bg-muted/40 sm:hidden" />
      {children}
